...
@interface NNFilterView ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation NNFilterView

#pragma mark - - LifeCycle

#pragma mark - - sharedInstance
+ (instancetype)shared{
    static NNFilterView * instance = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[NNFilterView alloc] init];
    });
    return instance;
}
...
- (void)setDataList:(NSArray *)dataList{
    ...
    for (NSDictionary * dic in _dataList) {
        NSParameterAssert([dic[kItemObj] count] == [dic[kItemObjSeleted] count]);
        
        NSDictionary * params = @{
                               kGviewItems :   dic[kItemObj],
                               kGviewItemsSelected :   dic[kItemObjSeleted],
        ...
        NNGView * view = [NNGView viewRect:rect params:params];
...
- (void)handleActionSender:(UIButton *)sender{
    if (sender.tag == 0) {
        ...
        [self.dataList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            NSMutableArray * arrSelected = dic[kItemObjSeleted];
        ...
        [self.viewList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NNGView * view = obj;
            view.selectedList = self.dataList[idx][kItemObjSeleted];
            [view layoutSubviews];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NNGView * view = self.viewList[indexPath.section];
...
//    cell.textLabel.textAlignment = NSTextAlignmentCenter;
    ...
    view.block = ^(NNGView *view, id item, NSInteger idx) {
//        DDLog(@"___%@,%@,%@",@(indexPath.section),@(idx),view.selectedList);
        NSMutableArray * marr = [self itemAtSection:indexPath.section key:kItemObjSeleted];
