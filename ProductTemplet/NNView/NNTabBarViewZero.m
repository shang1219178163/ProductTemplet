...
@interface NNTabBarViewZero()<UIScrollViewDelegate>

@property (strong, nonatomic, readwrite) NSArray *items;

@end

@implementation NNTabBarViewZero

+ (NNTabBarViewZero *)viewRect:(CGRect)frame items:(NSArray *)items{
    NNTabBarViewZero * view = [[NNTabBarViewZero alloc]initWithFrame:frame items:items];
    return view;
