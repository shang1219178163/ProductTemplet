...
- (void)createAlertView{
    ...
    NNAlertViewOne * alertView = self.alertView;
    alertView.block = ^(NNAlertViewOne *view, NSIndexPath *indexPath) {
        NSArray * array = view.data[kItemObj];
    ...
    if (self.dic && [self.dic isKindOfClass:[NSDictionary class]]) {
        ...
        alertView.data = @{kItemObj: array,
                           };
        [alertView show];
    }
    else if ([alertView.data[kItemObj] isKindOfClass:[NSArray class]]){
        [alertView show];
...
-(NNAlertViewOne *)alertView{
    if (!_alertView) {
        _alertView = [[NNAlertViewOne alloc]init];
