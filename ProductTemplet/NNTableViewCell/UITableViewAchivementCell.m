...
- (void)createControls{
    ...
    [itemListSub enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NNPairLabelView * pairView = [[NNPairLabelView alloc] initWithFrame:CGRectZero];
        pairView.labOne.textColor = [UIColor colorWithWhite:0.0 alpha:0.5];
        pairView.labOne.text = obj;
        pairView.labTwo.text = kNilText;
...
//    
    ...
    NSString *numDecStyleStr = [NSNumberFormatter localizedString:NSNumberFormatterDecimalStyle number:[self.valueList firstObject]];
    ...
    [self.viewList enumerateObjectsUsingBlock:^(NNPairLabelView *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NNMonthView *)monthView{
    if (!_monthView) {
        _monthView = ({
            NNMonthView * view = [[NNMonthView alloc]initWithFrame:CGRectZero];
...
-(NNPairLabelView *)pairView{
    if (!_pairView) {
        _pairView = [[NNPairLabelView alloc]initWithFrame:CGRectZero];
