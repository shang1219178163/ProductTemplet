...
-(NNAddPictureView *)pictureView{
    if (!_pictureView) {
        _pictureView = ({
            CGFloat itemWidth = (CGRectGetWidth(self.frame) - (kRowOfNumber - 1)*kPadding)/kRowOfNumber;
            NNAddPictureView * pictureView = [[NNAddPictureView alloc]initWithFrame:CGRectMake(kX_GAP, kY_GAP, self.contentView.sizeWidth - kX_GAP*2 , itemWidth)];
