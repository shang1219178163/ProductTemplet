...
-(void)layoutSubviews{
    ...
    CGSize contentSize = [self sizeWithText:self.labelLeftSub.text font:self.labelLeftSub.font width:(self.contentView.sizeWidth - 2*kX_GAP)];

    //控件1
    CGRect labelLeftRect = CGRectMake(kX_GAP, kY_GAP, textSize.width, kH_LABEL);
    CGRect labelLeftMarkRect = CGRectMake(CGRectGetMaxX(labelLeftRect) + kPadding, kY_GAP, self.contentView.sizeWidth - CGRectGetMaxX(labelLeftRect) - kPadding - kX_GAP, kH_LABEL);

    CGFloat height = self.contentView.maxY - kY_GAP*2 - kPadding - kH_LABEL;
    //控件2
    CGRect labelLeftSubRect = CGRectMake(kX_GAP, CGRectGetMaxY(labelLeftRect) + kPadding, self.contentView.sizeWidth - 2*kX_GAP, height);
