...
-(void)layoutSubviews{
    ...
    if (self.labelLeft.text.isEmpty) self.labelLeft.hidden = YES;;
    if (self.labelRight.text.isEmpty) self.labelRight.hidden = YES;
...
-(UIView *)lineVert{
    if (!_lineVert) {
        _lineVert = [UIView createLineRect:CGRectMake(0, 0, self.contentView.sizeWidth, kH_LINE_VIEW) isDash:NO hidden:NO];
        _lineVert.tag = kTAG_VIEW+11;
...
-(UILabel *)labelRightSub{
    if (!_labelRightSub) {
        _labelRightSub = [UILabel createRect:CGRectZero type:NNLabelTypeFitWidth];
