...
-(void)layoutSubviews{
    ...
    //控件2
    CGRect labelLeftRect = CGRectMake(CGRectGetMaxX(iConRect) + padding, kY_GAP, self.contentView.sizeWidth - CGRectGetMaxX(iConRect) - padding - kX_GAP, kH_LABEL_SMALL);
    //控件3
...
-(UIImageView *)iConEP{
    if (!_iConEP) {
        _iConEP = ({
            UIImageView * view = [UIImageView createRect:CGRectZero];
            view.image = [UIImage imageNamed:@"icon_company.png"];
            view;
        });
    }
    return _iConEP;
