...
@implementation BNMap

#pragma mark - - sharedInstance
+ (instancetype)sharedInstance {
    static BNMap * _singleton = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _singleton = [[BNMap alloc] init];
    });
    return _singleton;
}

+(instancetype)allocWithZone:(struct _NSZone *)zone{
    static BNMap *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
+ (NSArray *)polylinesForPath:(AMapPath *)path
{
    ...
    [path.steps enumerateObjectsUsingBlock:^(AMapStep *step, NSUInteger idx, BOOL *stop) {
        NSUInteger count = 0;
        CLLocationCoordinate2D *coordinates = [BNMap coordinatesForString:step.polyline
...
- (MAMapView *)mapView{
    if (!_mapView) {
        _mapView = [[MAMapView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight - kNaviBarHeight)];
