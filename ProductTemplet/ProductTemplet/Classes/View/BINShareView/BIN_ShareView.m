...
static const CGFloat padding = 8.0;

@interface BIN_ShareView ()
...
@implementation BIN_ShareView

+ (BIN_ShareView *)shareViewWithShareModels:(NSArray *)shareModels topTitle:(NSString *)topTitle btnTitle:(NSString *)btnTitle{
    
    BIN_ShareView *view = [[self alloc]initWithShareModels:shareModels topTitle:topTitle btnTitle:(NSString *)btnTitle];
    return view;
}
...
- (void)shareBtnPress:(UIButton *)sender{
    ...
    [self tappedCancel];
    if (self.block) {
        self.block(sender.tag - kTAG_VIEW);
    }
}
...
-(UIView *)lineView{
    ...
    if (_lineView == nil) {
        ...
        _lineView.backgroundColor = kC_LineColor;
