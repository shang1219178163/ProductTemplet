...
- (void)registerForKVO{
    ...
    [self.KVOController observe:self keyPath:@"frame" options:NSKeyValueObservingOptionOld|NSKeyValueObservingOptionNew block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
        //        DDLog(@"监听:%@",change);
        NSString *keyPath = change[FBKVONotificationKeyPathKey];
        if([keyPath isEqualToString:@"frame"] && !CGRectEqualToRect(CGRectZero,self.frame)){
            [self updateFrame];
    ...
    [self.KVOController observe:self.imageView keyPath:@"image" options:NSKeyValueObservingOptionOld|NSKeyValueObservingOptionNew block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
       
        NSString *keyPath = change[FBKVONotificationKeyPathKey];
        if([keyPath isEqualToString:@"image"]){
            self.imageView.hidden = self.imageView.image ? NO : YES;
//            DDLog(@"hidden__%@",@(self.imageView.hidden));
