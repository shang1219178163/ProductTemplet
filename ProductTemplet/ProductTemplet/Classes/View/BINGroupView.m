    ...
    if (self) {
        ...
        for (NSInteger i = 0; i< items.count; i++) {
            ...
            if ([self.selectedList containsObject:title]) {
                [self handleSender:btn backgroudColor:kC_ThemeCOLOR textColor:[UIColor whiteColor] layerColor:kC_ThemeCOLOR];
...
//        NSAssert(isOnlyOne == YES && self.selectedList.count == 1, @"isOnlyOne = yes代表单选");
        ...
        for (UIButton *sender in self.subviews) {
            ...
            if ([self.selectedList containsObject:title]) {
                [self handleSender:sender backgroudColor:kC_ThemeCOLOR textColor:[UIColor whiteColor] layerColor:kC_ThemeCOLOR];
...
-(void)setColorBackSelected:(UIColor *)colorBackSelected{
    for (UIButton *sender in self.subviews) {
        if ([self.selectedList containsObject:sender.titleLabel.text]) {
            [self handleSender:sender backgroudColor:colorBackSelected textColor:[UIColor whiteColor] layerColor:kC_ThemeCOLOR];
...
//
//    [sender setBackgroundImage:[UIImage imageWithColor:kC_ThemeCOLOR] forState:UIControlStateNormal];
//    [sender setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
//
//    [sender showLayerColor:kC_ThemeCOLOR];
//
//}
