...
#import "BN_ReportView.h"

#import "NSNumberFormatter+Helper.h"

@interface BN_ReportView ()<UIScrollViewDelegate>
...
@property (nonatomic, strong) NSArray * pieItemList;
...
@implementation BN_ReportView

-(NSArray *)pieItemList{
    if (!_pieItemList) {
        _pieItemList = @[
                         @{ kPie_Title :    @"item0",   kPie_Value : @10 },
                         @{ kPie_Title :    @"item1",   kPie_Value : @20 },
                         @{ kPie_Title :    @"item2",   kPie_Value : @30 },
                         @{ kPie_Title :    @"item3",   kPie_Value : @40 },
                         @{ kPie_Title :    @"item4",   kPie_Value : @50 },
                         @{ kPie_Title :    @"item5",   kPie_Value : @60 },
                         @{ kPie_Title :    @"item6",   kPie_Value : @70 },
                         @{ kPie_Title :    @"item7",   kPie_Value : @80 },
                         @{ kPie_Title :    @"item8",   kPie_Value : @90 },
                         ];
    }
    return _pieItemList;
}
...
- (void)createControls{
    ...
    for (NSInteger i = 0; i < self.heightList.count; i++) {
        if (i == 0) {
            rect = CGRectMake(0, 0, CGRectGetWidth(self.bounds), [self.heightList[i] floatValue]);
            
        }else{
//            rect = CGRectMake(0, CGRectGetMaxY(rect) + _spacing, CGRectGetWidth(self.bounds), [self.heightList[i] floatValue] - _spacing);
            rect = CGRectMake(0, CGRectGetMaxY(rect) + _spacing, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - CGRectGetMaxY(rect) - _spacing);
...
//        view.backgroundColor = [UIColor randomColor];
    ...
    [self configureTheExcelView];
}

- (void)configureTheChartView:(AAChartType)chartType {
    UIView * view = self.viewList[0];
    
    self.aaChartView = [[AAChartView alloc]initWithFrame:view.frame];
    
    self.aaChartView.scrollEnabled = NO;
    self.aaChartView.contentHeight = self.aaChartView.frame.size.height;
//    DDLog(@"%@__%@",NSStringFromCGRect(self.aaChartView.frame),@(self.aaChartView.contentHeight));

//    self.aaChartView.backgroundColor = [UIColor randomColor];
...
- (AAChartModel *)configureTheChartModel:(NSString *)chartType {
    if ([chartType isEqualToString:@"colorfulColumnChart"]) {
        ...
        .colorsThemeSet([self.colorList subarrayWithRange:NSMakeRange(0, self.pieItemList.count)])
        .titleSet(@"")
        .subtitleSet(@"")
...
- (void)configureTheExcelView{
    UIView * view = self.viewList[1];
    self.excelView.frame = CGRectMake(0, 0, kScreen_width, CGRectGetHeight(view.frame));
...
- (void)updateChartViewWidth:(CGFloat)width{
    ...
    CGRect rect = self.aaChartView.frame;
    rect.size.width = width;
    self.aaChartView.frame = rect;
    
    UIScrollView * scrollView = (UIScrollView *)[self.aaChartView superview];
    scrollView.contentSize = CGSizeMake(width, CGRectGetHeight(scrollView.frame));
}

- (NSArray *)BN_filterModelList:(NSArray *)modelList query:(NSString *)query{
...
-(NSArray *)colorList{
    if (!_colorList) {
        _colorList = @[
                       @"#7CC7EF",@"#EE827B",@"#6D6DB7",
                       @"#EF7BCC",@"#9DC659",@"#EFC57B",
                       @"#EAC500",@"#10A4CF",@"#D06C89",
                       ];
...
-(BN_ExcelView *)excelView{
    if (!_excelView) {
        CGFloat spacing = 0;
        CGRect rect = CGRectMake(spacing, spacing, kScreen_width - spacing*2, CGRectGetHeight(self.bounds) - spacing*2);
...
-(NSMutableArray *)viewList{
    if (!_viewList) {
        _viewList = [NSMutableArray arrayWithCapacity:0];
    }
    return _viewList;
}
...
-(UIScrollView *)scrollView{
    if (!_scrollView) {
        ...
        _scrollView.backgroundColor = kC_BackgroudColor;

        _scrollView.pagingEnabled = YES;
        _scrollView.delegate = self;
      
        _scrollView.contentSize = CGSizeMake(CGRectGetWidth(self.containView.frame), CGRectGetHeight(self.containView.frame));
...
-(NSArray *)heightList{
    if (!_heightList) {
        _heightList = @[@(kH_ChartView),@(kScreen_width)];
    }
    return _heightList;
}
...
-(UIView *)containView{
    if (!_containView) {
        // 初始化
        NSNumber *sum = [self.heightList valueForKeyPath:@"@sum.floatValue"];
        
        CGRect rect = CGRectMake(0, 0, kScreen_width, [sum floatValue]);
