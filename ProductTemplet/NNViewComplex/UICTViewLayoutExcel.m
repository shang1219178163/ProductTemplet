...
@implementation UICTViewLayoutExcel
    ...
    if (self.itemAttributes.count > 0) {
        for (NSInteger section = 0; section < self.collectionView.numberOfSections; section++) {
            ...
            for (NSUInteger row = 0; row < numberOfItems; row++) {
                ...
                [self p_layoutAtt:attributes indexPath:indexPath];
            }
        }
        return;
    }
...
- (void)p_layoutAtt:(UICollectionViewLayoutAttributes *)layoutAtt indexPath:(NSIndexPath *)indexPath{
    //第一行
